...
## Punctuation
NEVER use em dashes (—), en dashes (–) or double hyphens (--). Use a plain hyphen "-" or restructure the sentence.

## Body images (use the real archive)
Both body images use markdown format `![Descriptive alt text](/images/posts/FILENAME)` and MUST be chosen from this list (do NOT invent filenames; pick ones that fit the person's story):
...
For exchange-related figures you may also use `/images/exchanges/{slug}.png` for exchanges that exist in the database.
