...
exchanges: ["Coinbase", "Binance", "Kraken", "Uniswap", "OKX"]
...
Policy engines then apply business logic. Categories like "utilities," "travel," and "cloud" can have separate spend caps. Time-based rules prevent odd-hour fraud. Destination lists restrict payments to known merchants or exchange deposit addresses. These checks should run before every signature and should be simple enough to audit. For context on the broader programmable money landscape, review **[What is DeFi?](/posts/what-is-defi)** to see how permissioned logic coexists with open protocols.
...
Centralized exchanges offer deep liquidity, reliable APIs, and fiat on-ramps. Teams often integrate with **[Coinbase](/exchanges/coinbase)**, **[Binance](/exchanges/binance)**, or **[Kraken](/exchanges/kraken)** for conversions and refills. API keys should be restricted to "trade only," with withdrawal addresses allowlisted. Since CEX accounts are custodial, KYC is required, and compliance rules apply.

Decentralized exchanges run on-chain smart contracts where the agent swaps tokens directly via an AMM or aggregator. Integrations with venues like **[Uniswap](/exchanges/uniswap)** avoid custodial risk and are programmatically transparent. The cost-benefit depends on trade size, slippage, and gas. For broad exchange considerations, see **[Understanding Crypto Exchanges](/posts/understanding-crypto-exchanges)** to compare features and risks. Some teams also maintain an account at **[OKX](/exchanges/okx)** or similar for overflow capacity or region-specific access.
...
Compliance is about predictable guardrails. Maintain logs that map every payment to a user-session, intent, and invoice hash. Screen addresses when required by jurisdiction. If you work with fiat conversion, align with travel rule policies from your exchange partner. Central bank digital currency pilots are evolving - our **[CBDCs overview](/posts/central-bank-digital-currencies)** explains what could change when state-issued digital cash enters the picture. Finally, social-engineering remains the top failure mode; a quick refresher on **[how to avoid crypto scams](/posts/how-to-avoid-crypto-scams)** is well worth your time.
...
From a UX perspective, the agent should display a human-friendly summary - "Paying 12.50 USDC to acme.eth on Arbitrum, fee $0.04" - and give you a cancel window or approval step above thresholds. Refunds can be automated with escrow contracts that release funds only on delivery or milestone checks. For traditional comparisons, our **[What is Ethereum?](/posts/what-is-ethereum)** primer covers the contract ecosystems that make these flows possible.
...
![Futuristic UI showing an AI agent approving and signing a crypto transaction](/images/posts/futuristic-ui.jpg)
...
If you are experimenting, start small: set a $20 daily cap, pay a real invoice in a stablecoin on a Layer 2, and review the on-chain receipt. As you grow, layer in MPC, session keys, and stricter allowlists. For foundational refreshers, compare assets in What Are Stablecoins? and revisit account models in What is Ethereum?. With the right guardrails, you can let software handle the busywork of payments while you stay squarely in control.